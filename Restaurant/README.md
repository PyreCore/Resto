...
- **Administration** : `ADMIN_MOT_DE_PASSE` (mot de passe unique, à changer en production) vérifié via `ADMIN_MOT_DE_PASSE_HASH` (hash généré par `password_hash`, à régénérer si le mot de passe change) et `ADMIN_EXPIRATION` (durée de la session admin, 300 s = 5 min).
...
- **Connexion admin** : mot de passe vérifié par `password_verify()` sur le hash `ADMIN_MOT_DE_PASSE_HASH` (repli `hash_equals` si non défini), **anti force-brute** (blocage temporaire de 5 minutes après 5 échecs successifs, géré dans `admin/login.php`), identifiant de session renouvelé à la connexion (`session_regenerate_id`).
- **Protection CSRF** : un jeton aléatoire par session (`csrf_token()`/`csrf_valider()` dans `config.php`) est exigé dans tous les formulaires (connexion, commande, plats, tables, couleurs) et dans les liens d'action de l'administration (validation, changement de statut, suppression). Toute requête sans jeton valide est refusée.
...
- **Garde-fou du dépôt Git** : nouveau fichier `C:\wamp64\www\.gitignore` (racine du dépôt) qui ignore **tout sauf le dossier `Restaurant/`**. Un futur `git add .` à la racine ne peut plus embarquer les autres dossiers WAMP (`wordpress/`, `Ruben/`, `wampthemes/`, etc.) ni les fichiers du serveur local. Le `.gitignore` interne de `Restaurant/` (logs, backups, config.php, .idea) reste appliqué.
- **Protection CSRF** : ajout dans `config.php` des fonctions `csrf_token()` (jeton aléatoire stocké en session, `bin2hex(random_bytes(32))`) et `csrf_valider()` (comparaison à temps constant via `hash_equals`, jeton lu dans `POST` ou `GET`). Un champ caché `csrf` est inséré dans tous les formulaires : connexion (`admin/login.php`), commande (`commande.php`), gestion des plats et des tables (`admin/plats.php`, les 2 formulaires) et couleurs (`admin/parametres.php`). Les liens d'action de l'administration transportent le jeton en paramètre d'URL : validation/changement de statut/annulation (`admin/index.php`), suppression d'un plat et retrait d'une table (`admin/plats.php`). Chaque page de traitement refuse la requête (« Session expirée. Veuillez réessayer. ») si le jeton est absent ou invalide : `admin/login.php`, `admin/plats.php`, `admin/parametres.php`, `admin/index.php`, `commandes-ajout.php`. Vérifié par tests HTTP (POST et actions GET sans jeton rejetés, données non modifiées).
- **Hashage du mot de passe administrateur** : le mot de passe n'est plus vérifié en clair. `config.php` définit `ADMIN_MOT_DE_PASSE_HASH` (résultat de `password_hash('admin123', PASSWORD_DEFAULT)`). `admin/login.php` utilise `password_verify()` sur ce hash ; `ADMIN_MOT_DE_PASSE` n'est conservé que comme repli de compatibilité si le hash est absent. Pour changer le mot de passe : modifier `ADMIN_MOT_DE_PASSE`, générer le nouveau hash (`php -r "echo password_hash('...', PASSWORD_DEFAULT);"`) et le reporter dans `ADMIN_MOT_DE_PASSE_HASH`.
- **Correction de la redirection de déconnexion** : dans `admin/login.php`, la cible de retour (`?retour=`) n'était pas validée (`header('Location: ' . $_GET['retour'])`), ce qui permettait une redirection ouverte et une éventuelle injection d'en-tête. Le retour est désormais limité à une liste blanche (`login.php`, `index.php`, `../commande.php`, `../mes-commandes.php`) ; toute autre valeur renvoie vers `login.php`.
